...
if (isset($_POST['check'])) {
    ...
    if($category != ''){
        $queries = "SELECT * FROM $check WHERE UNIX_TIMESTAMP(dateofupload) > 'UNIX_TIMESTAMP($movieid)' AND hdversion != 'Movie Request' AND genres LIKE '%$category%' ORDER BY dateofupload DESC, id DESC, REPLACE(imdb, '.', '')+0 DESC,STR_TO_DATE(release_date, '%Y-%m-%d') DESC,REPLACE(rtomatoes, '.', '')+0 DESC LIMIT $starting, 21";
    }elseif($searchvalue != ''){
	    $queries = "SELECT moviename,imdb,descof_video,thumbnail,movie_desc,release_date,hdversion,dateofupload FROM movies WHERE moviename LIKE '%$searchvalue%' OR movie_desc LIKE '%$searchvalue%' UNION ALL SELECT moviename,imdb,descof_video,thumbnail,movie_desc,release_date,hdversion,dateofupload FROM series WHERE moviename LIKE '%$searchvalue%' OR movie_desc LIKE '%$searchvalue%' LIMIT $starting,21;";   
    }else{
      $queries = "SELECT * FROM $check WHERE UNIX_TIMESTAMP(dateofupload) > 'UNIX_TIMESTAMP($movieid)' AND hdversion != 'Movie Request' ORDER BY dateofupload DESC, REPLACE(imdb, '.', '')+0 DESC,STR_TO_DATE(release_date, '%Y-%m-%d') DESC,REPLACE(rtomatoes, '.', '')+0 DESC LIMIT $starting, 21";   
    }
...
<g>
	...
	?>
            <img loading="lazy" class = "img-fluid" src= "admin/<?php echo $rows['thumbnail']?>" alt="<?php echo $rows['moviename']?>">
